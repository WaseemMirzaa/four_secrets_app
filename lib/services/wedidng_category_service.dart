...
class WeddingCategoryDatabase {
  ...
  Future<void> createInitialWeddingCategories() async {
    ...
    final bool isInitialized = userData?['weddingCategoriesInitialized'] ?? false;
    ...
    if (!isInitialized) {
      ...
      print("Initial wedding categories created successfully and marked as initialized");
    } else {
      print("User's wedding categories were already initialized, skipping creation");
    }
  }
  ...
  Future<List<WeddingCategoryModel>> loadWeddingCategories() async {
    ...
    try {
      ...
      if (!userDoc.exists || !(userDoc.data()?['weddingCategoriesInitialized'] ?? false)) {
      ...
      if (snapshot.docs.isEmpty) {
        print("Warning: No wedding categories found even though collection should be initialized");
      }
      ...
      print("Loaded ${categoryList.length} wedding categories for user: $userId");
      return categoryList;
    } catch (e) {
  ...
  Map<String, List<String>> searchCategories(String query) {
    ...
    for (var category in categoryList) {
      ...
      bool categoryMatches = category.categoryName.toLowerCase().contains(lowerQuery);
      ...
      if (categoryMatches || matchedItems.isNotEmpty) {
        filteredMap[category.categoryName] = categoryMatches ? category.items : matchedItems;
      }
    }
